...
1. Understand what types of data are best modeled by different distributions
    ...
    + normal
    + poisson
2. Be able to generate and plot data randomly sampled from the above distributions
3. Be able to test sampled distributions against a null hypothesis
    + exact binomial test
...
4. Create a function to generate a sample with specific properties and run an inferential test
5. Calculate power using `replicate` and a sampling function
6. Calculate the minimum sample size for a specific power level and design

### Advanced

7. Generate 3+ variables from a multivariate normal distribution and plot them
...
<p class="alert alert-info">Run the code above several times. How often do you 
conclude the fair coin is biased (false positives)? How often do you conclude 
the biased coin is fair (false negatives)?</p>
...
Now replicate this 10000 times using the `replicate()` function. This function 
...
This design has about 60% power to detect the sex difference in height (with a 
...
increase the number of replications from 1000 to 10000.
...
1. Generate a dataset with one normally distributed variable with a mean of 0 
and SD of 1 (`a <- rnorm(n, 0, 1`) and another variable that is equal to the sum 
of `a` and another normally distributed variable with a mean of 0.5 and SD of 1 
(`b <- a + rnorm(n, 0.5, 1)`). Run paired-samples t-tests comparing `a` and `b` 
...
2. Calculate power for a two-tailed t-test with an alpha of .05 for detecting 
a difference between two independent samples of 100 with an effect size of 0.2?
...
3. Modify the `sim_t_ind` function to handle different 
...
4. Calculate power for a two-tailed t-test with an alpha of .005 for detecting 
...
5. What is the poisson distribution? How do you sample from a poisson distribution?
    ...
    weekends than weekdays. The poisson distributiongets more like a normal 
...
6. Prove to yourself that the binomial distribution looks like the normal 
...
=======
<<<<<<< HEAD
>>>>>>> f263a2c0058ee93f683cada986969419867b9833
    <p class="alert alert-info">You can calculate the equivalent mean for the normal 
    distribution as the number of trials times the probability of success 
    (`mean <- trials * prob`) and the equivalent SD as the square root of the mean 
    times one minus probability of success (`sd <- sqrt(mean * (1 - prob))`).</p>
<<<<<<< HEAD
=======
=======
<p class="alert alert-info">You can calculate the equivalent mean for the normal 
...
(`mean <- trials * prob`) and the equivalent SD as the square root of the mean 
times one minus probability of success (`sd <- sqrt(mean * (1 - prob))`).</p>
>>>>>>> 0528fe831db42324e72fe4ebe354975e82cc891e
>>>>>>> f263a2c0058ee93f683cada986969419867b9833
