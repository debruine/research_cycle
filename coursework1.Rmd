...
3. How many people completed both questionnaires on the same day?

```{r, echo = FALSE, eval = FALSE}

emp_d <- mutate(emp, date = substr(endtime, 1, 10))
sys_d <- mutate(sys, date = substr(endtime, 1, 10))
...
4. Create a table of demographic information 
    ...
    Include (N, mean age, sd age, number missing age) by sex, only for people who completed both questionnaires on the same day.
...
5. Create EQ and SQ scores

    Create a table of mean EQ and SQ by sex for people who completed both 
    questionnaires on the same day. Include how many people have a missing score.
...
6. Graph the distribution of EQ and SQ for men and women

```{r, echo = FALSE, eval = FALSE}
emp_sys_scores %>%
  filter(sex %in% c("male", "female")) %>%
  ggplot(aes(score, fill = questionnaire)) + 
  geom_density(alpha = 0.5) +
  facet_grid(.~sex)
```
