...
4. Be able to use arguments like `sep`, `extra`, and `convert` to handle less straighforward data cleaning

### Advanced

5. Be able to use [regular expressions](https://stat.ethz.ch/R-manual/R-devel/library/base/html/regex.html) to separate complex columns
...
This example is complicated because the column names to gether _are_ numbers. 
...
<p class="fyi">The previous example should have given you an error warning about 
...
<p class="fyi">You can use [regular expressions](https://stat.ethz.ch/R-manual/R-devel/library/base/html/regex.html) 
...
Nope, that didn't work at all, but it's a really common mistake when spreading 
...
`rate`, `ci_low` and `ci_hi` columns back together.
...
<p class="fyi">What if you want to put it back into the format "rate [ci_low - ci_hi]"? 
...
Load the data and convert from wide to long format. 
The resulting dataframe should have the columns: 
`user_id`, `date`, `question`, and `score`.
...
split "abcde" after the third characters, use `sep = 3`, which results in 
c("abc", "de").
...
Put the domain and qnumber columns back together into a new column named `domain_n`. 
