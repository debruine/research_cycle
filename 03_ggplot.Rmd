...
disgust <- read_csv("data/disgust.csv") %>%
  gather("question", "rating", moral1:pathogen7) %>%
  mutate(domain = substr(question, 1, nchar(question) - 1)) %>%
  group_by(id, date, user_id, domain) %>%
  summarise(score = mean(rating)) %>%
  spread(domain, score) %>%
  ggplot(aes(moral, sexual)) +
  geom_point()
...
Creat a heatmap of the relationships among all the questions in the [Disgust Dataset](data/disgust.csv). 

```{r, echo = FALSE}
disgust <- read_csv("data/disgust.csv")

heatmap <- disgust %>%
  select(moral1:pathogen7) %>%
  filter(!is.na(rowSums(.))) %>%
  cor() %>% # create the correlation matrix
  as.data.frame() %>% # make it a data frame
  rownames_to_column(var = "V1") %>% # set rownames as V1
  gather("V2", "r", moral1:pathogen7) # wide to long (V2)
