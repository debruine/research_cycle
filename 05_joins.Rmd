...
You can combine the rows of two table with `bind_rows`.
...
<p class="fyi">Notice that the `by` argument now needs to specify both the left 
...
joined table and any data from the "left" table that matches). Remember to switch 
...
6. Create a table of data from users who did not complete either thepersonality questionnair or the disgust questionnaire. (_Hint: this will require two steps; use pipes._)
...
9. How many users are there in total across the first and second user tables?
